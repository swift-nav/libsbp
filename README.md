...
comparison to the native construct-based bindings included in libsbp.
However, due to the nature of Kaitai Struct, the bindings do not allow for
serialisation (i.e. construction of SBP objects).  It should also be noted
that the names used by the KSY bindings are not 1:1 identical to the native
bindings due constraints which Kaitai Struct places on various token
types (e.g. it is forbidden to use upper case characters in field names).
