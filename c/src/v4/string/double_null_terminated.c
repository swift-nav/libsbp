...
void sbp_double_null_terminated_string_init(sbp_string_t *s, size_t max_encoded_len)
{
...
bool sbp_double_null_terminated_string_valid(const sbp_string_t *s, size_t max_encoded_len)
{
  if (s->encoded_len < 2) return false;
...
size_t sbp_double_null_terminated_string_encoded_len(const sbp_string_t *s, size_t max_encoded_len)
{
  if (!sbp_double_null_terminated_string_valid(s, max_encoded_len)) return 2;
...
size_t sbp_double_null_terminated_string_space_remaining(const sbp_string_t *s,
                                             size_t max_encoded_len)
{
  return max_encoded_len - sbp_double_null_terminated_string_encoded_len(s, max_encoded_len);
}

size_t sbp_double_null_terminated_string_count_sections(const sbp_string_t *s,
                                            size_t max_encoded_len)
{
  if (!sbp_double_null_terminated_string_valid(s, max_encoded_len)) return 0;
  if (s->encoded_len == 2) return 0;
  // Only count up to one less than the encoded len to avoid the extra NULL terminator, count NULL terminators
  size_t sections = 0;
                                      ...
                                      const char *str)
{
  ...
  if (s->encoded_len == 2)
  {
    if (!sbp_string_copy_to_buf(s->data, &copied, max_encoded_len - 1, str)) return false;
    s->encoded_len = copied + 1; // Extra null terminator
    return true;
  }

  if (!sbp_string_copy_to_buf(s->data + s->encoded_len - 1, &copied, max_encoded_len - s->encoded_len + 1, str)) return false;
                                              ...
                                              va_list ap)
{
  ...
  if (s->encoded_len == 2)
  {
    if (!sbp_string_vprintf_to_buf(s->data, &copied, max_encoded_len - 1, fmt, ap)) return false;
...
{
  maybe_init(s, max_encoded_len);
  if (s->encoded_len == 2) return sbp_double_null_terminated_string_add_section(s, max_encoded_len, new_str);

  size_t copied;
  if (!sbp_string_copy_to_buf(s->data + s->encoded_len - 2, &copied, max_encoded_len - s->encoded_len + 1, new_str)) return false;
  s->encoded_len += copied - 1;
  return true;
}
...
{
  maybe_init(s, max_encoded_len);
  if (s->encoded_len == 2) return sbp_double_null_terminated_string_add_section_vprintf(s, max_encoded_len, fmt, ap);

  size_t copied;
  if (!sbp_string_vprintf_to_buf(s->data + s->encoded_len - 2, &copied, max_encoded_len - s->encoded_len + 1, fmt, ap)) return false;
  s->encoded_len += copied - 1;
  return true;
}
...
const char *sbp_double_null_terminated_string_get_section(const sbp_string_t *s,
                                             ...
                                             size_t section)
{
  if (!sbp_double_null_terminated_string_valid(s, max_encoded_len)) return NULL;
  size_t offset = section_offset(s, section);
  if (offset == SIZE_MAX) return NULL;
  return s->data + offset;
}

bool sbp_double_null_terminated_string_encode(const sbp_string_t *s, size_t max_encoded_len, sbp_encode_ctx_t *ctx)
{
  return sbp_string_encode(s, max_encoded_len, ctx, &params);
}

bool sbp_double_null_terminated_string_decode(sbp_string_t *s, size_t max_encoded_len, sbp_decode_ctx_t *ctx)
{
