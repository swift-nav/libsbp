...
bool sbp_unterminated_string_valid(const sbp_string_t *s,
  ...
  if (n > max_encoded_len) return false;
  if (n + 1 == s->encoded_len && s->data[n] == 0) return true;
...
size_t sbp_unterminated_string_encoded_len(const sbp_string_t *s,
                                           size_t max_encoded_len) {
  if (!sbp_unterminated_string_valid(s, max_encoded_len) || s->encoded_len == 0)
    return 0;
...
bool sbp_unterminated_string_set(sbp_string_t *s, size_t max_encoded_len,
                                 const char *new_str) {
  size_t copied;
  if (!sbp_string_copy_to_buf(s->data, &copied, max_encoded_len + 1u, new_str))
    return false;
...
bool sbp_unterminated_string_append_vprintf(sbp_string_t *s,
  ...
  if (!sbp_string_vprintf_to_buf(s->data + s->encoded_len, &copied,
                                 max_encoded_len - s->encoded_len + 1, fmt, ap))
    return false;
  s->encoded_len += copied - 1;
  return true;
}
...
bool sbp_unterminated_string_pack(const sbp_string_t *s, size_t max_encoded_len,
                                  sbp_encode_ctx_t *ctx) {
  return sbp_string_encode(s, max_encoded_len, ctx, &params);
}

bool sbp_unterminated_string_unpack(sbp_string_t *s, size_t max_encoded_len,
