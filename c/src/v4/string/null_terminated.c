...
{
  if (!sbp_null_terminated_string_valid(s, params) || s->encoded_len == 0)
...
uint8_t sbp_null_terminated_string_strlen(const sbp_null_terminated_string_t *s,
                                          const sbp_null_terminated_string_params_t *params)
{
  return (uint8_t)(sbp_null_terminated_string_encoded_len(s, params) - 1);
}

uint8_t sbp_null_terminated_string_space_remaining(const sbp_null_terminated_string_t *s,
                                                   const sbp_null_terminated_string_params_t *params)
{
  return (uint8_t)(params->max_encoded_len - sbp_null_terminated_string_encoded_len(s, params));
}
                                    ...
                                    const char *new_str)
{
  ...
  s->encoded_len = (uint8_t)(new_str_len + 1);
                                       ...
                                       const char *new_str)
{
  ...
  uint8_t offset = (uint8_t)(s->encoded_len - 1);
  strncpy(s->data + offset, new_str, max_copy);
  s->encoded_len = (uint8_t)(s->encoded_len + max_copy - 1);
  return true;
}
...
{
  if ((ctx->buf_len - ctx->offset) < sbp_null_terminated_string_encoded_len(s, params))
    return false;
  if (!sbp_null_terminated_string_valid(s, params) || s->encoded_len == 0)
  {
                                       ...
                                       sbp_decode_ctx_t *ctx)
{
  ...
  // - A NULL is found up to params->max_encoded_len into the incoming buffer
  // - The buffer contains less than params->max_encoded_len bytes and there is a NULL terminator in the final byte
  // - The buffer contains less than params->max_encoded_len bytes and there is no NULL terminator in the final byte
  ...
  if (i == max_copy)
  {
    ...
    // and call it success
    s->data[i] = 0;
    s->encoded_len = (uint8_t)(i + 1); // Append a NULL terminator
    ctx->offset += i;
    return true;
  }
