...
bool sbp_msg_ed25519_signature_dep_encode_internal(
  ...
  for (size_t i = 0; i < SBP_MSG_ED25519_SIGNATURE_DEP_SIGNATURE_MAX; i++) {
    if (!sbp_u8_encode(ctx, &msg->signature[i])) {
      return false;
    }
  }
  for (size_t i = 0; i < SBP_MSG_ED25519_SIGNATURE_DEP_FINGERPRINT_MAX; i++) {
...
s8 sbp_msg_ed25519_signature_dep_encode(
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ed25519_signature_dep_encode_internal(&ctx, msg)) {
...
bool sbp_msg_ed25519_signature_dep_decode_internal(
  ...
  for (uint8_t i = 0; i < SBP_MSG_ED25519_SIGNATURE_DEP_SIGNATURE_MAX; i++) {
    if (!sbp_u8_decode(ctx, &msg->signature[i])) {
      return false;
    }
  }
  for (uint8_t i = 0; i < SBP_MSG_ED25519_SIGNATURE_DEP_FINGERPRINT_MAX; i++) {
...
s8 sbp_msg_ed25519_signature_dep_decode(const uint8_t *buf, uint8_t len,
                                        uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ed25519_signature_dep_decode_internal(&ctx, msg)) {
...
s8 sbp_msg_ed25519_signature_dep_send(
    sbp_state_t *s, u16 sender_id, const sbp_msg_ed25519_signature_dep_t *msg,
    sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ed25519_signature_dep_encode(payload, sizeof(payload),
                                                &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_payload_send(s, SBP_MSG_ED25519_SIGNATURE_DEP, sender_id,
                          payload_len, payload, write);
}

int sbp_msg_ed25519_signature_dep_cmp(
    const sbp_msg_ed25519_signature_dep_t *a,
    const sbp_msg_ed25519_signature_dep_t *b) {
  int ret = 0;

  for (uint8_t i = 0;
       ret == 0 && i < SBP_MSG_ED25519_SIGNATURE_DEP_SIGNATURE_MAX; i++) {
  ...
  for (uint8_t i = 0;
       ret == 0 && i < SBP_MSG_ED25519_SIGNATURE_DEP_FINGERPRINT_MAX; i++) {
