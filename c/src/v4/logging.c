...
bool sbp_msg_log_text_append_printf(sbp_msg_log_t *msg, const char *fmt, ...) 
{
  ...
  bool sbp_msg_log_text_append_vprintf(sbp_msg_log_t *msg, const char *fmt, va_list ap)
{
...
bool sbp_msg_log_encode_internal(sbp_encode_ctx_t *ctx, const sbp_msg_log_t *msg)
{
  if (!sbp_u8_encode(ctx, &msg->level)) { return false; }
  if (!sbp_unterminated_string_encode(&msg->text, 254, ctx)) { return false; }
  return true;
}

s8 sbp_msg_log_encode(uint8_t *buf, uint8_t len, uint8_t *n_written, const sbp_msg_log_t *msg) {
...
{
  if (!sbp_u8_decode(ctx, &msg->level)) { return false; }
  if (!sbp_unterminated_string_decode(&msg->text, 254, ctx)) { return false; }
  return true;
}

s8 sbp_msg_log_decode(const uint8_t *buf, uint8_t len, uint8_t *n_read, sbp_msg_log_t *msg) {
...
s8 sbp_msg_log_send(struct sbp_state *s, u16 sender_id, const sbp_msg_log_t *msg, sbp_write_fn_t write)
{
...
size_t sbp_msg_fwd_encoded_len(const sbp_msg_fwd_t *msg) {
  ...
  encoded_len += (msg->n_fwd_payload * sbp_u8_encoded_len(&msg->fwd_payload[0]));
  return encoded_len;
}

bool sbp_msg_fwd_encode_internal(sbp_encode_ctx_t *ctx, const sbp_msg_fwd_t *msg)
{
  if (!sbp_u8_encode(ctx, &msg->source)) { return false; }
  if (!sbp_u8_encode(ctx, &msg->protocol)) { return false; }
  for (size_t i = 0; i < msg->n_fwd_payload; i++)
  {
    if (!sbp_u8_encode(ctx, &msg->fwd_payload[i])) { return false; }
  }
  return true;
}

s8 sbp_msg_fwd_encode(uint8_t *buf, uint8_t len, uint8_t *n_written, const sbp_msg_fwd_t *msg) {
  sbp_encode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
...
bool sbp_msg_fwd_decode_internal(sbp_decode_ctx_t *ctx, sbp_msg_fwd_t *msg)
{
  ...
  if (!sbp_u8_decode(ctx, &msg->protocol)) { return false; }
    msg->n_fwd_payload = (uint8_t)((ctx->buf_len - ctx->offset) / sbp_u8_encoded_len(&msg->fwd_payload[0]));
  for (uint8_t i = 0; i < msg->n_fwd_payload; i++) {
    if (!sbp_u8_decode(ctx, &msg->fwd_payload[i])) { return false; }
  }
  return true;
}

s8 sbp_msg_fwd_decode(const uint8_t *buf, uint8_t len, uint8_t *n_read, sbp_msg_fwd_t *msg) {
  sbp_decode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
...
s8 sbp_msg_fwd_send(struct sbp_state *s, u16 sender_id, const sbp_msg_fwd_t *msg, sbp_write_fn_t write)
{
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
...
int sbp_msg_print_dep_text_strcmp(const sbp_msg_print_dep_t *a, const sbp_msg_print_dep_t *b)
{
...
bool sbp_msg_print_dep_text_printf(sbp_msg_print_dep_t *msg, const char *fmt, ...) 
{
 ...
 bool sbp_msg_print_dep_text_vprintf(sbp_msg_print_dep_t *msg, const char *fmt, va_list ap)
{
  return sbp_unterminated_string_vprintf(&msg->text, 255, fmt, ap);
}

bool sbp_msg_print_dep_text_append_printf(sbp_msg_print_dep_t *msg, const char *fmt, ...) 
{
  va_list ap;
  va_start(ap, fmt);
  ...
  bool sbp_msg_print_dep_text_append_vprintf(sbp_msg_print_dep_t *msg, const char *fmt, va_list ap)
{
...
bool sbp_msg_print_dep_encode_internal(sbp_encode_ctx_t *ctx, const sbp_msg_print_dep_t *msg)
{
  if (!sbp_unterminated_string_encode(&msg->text, 255, ctx)) { return false; }
  return true;
}

s8 sbp_msg_print_dep_encode(uint8_t *buf, uint8_t len, uint8_t *n_written, const sbp_msg_print_dep_t *msg) {
  sbp_encode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
...
bool sbp_msg_print_dep_decode_internal(sbp_decode_ctx_t *ctx, sbp_msg_print_dep_t *msg)
{
  if (!sbp_unterminated_string_decode(&msg->text, 255, ctx)) { return false; }
  return true;
}

s8 sbp_msg_print_dep_decode(const uint8_t *buf, uint8_t len, uint8_t *n_read, sbp_msg_print_dep_t *msg) {
  sbp_decode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
...
s8 sbp_msg_print_dep_send(struct sbp_state *s, u16 sender_id, const sbp_msg_print_dep_t *msg, sbp_write_fn_t write)
{
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_print_dep_encode(payload, sizeof(payload), &payload_len, msg);
  if (ret != SBP_OK) { return ret; }
  return sbp_payload_send(s, SBP_MSG_PRINT_DEP, sender_id, payload_len, payload, write);
}

int sbp_msg_print_dep_cmp(const sbp_msg_print_dep_t *a, const sbp_msg_print_dep_t *b) {
  int ret = 0;
