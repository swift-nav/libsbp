...
bool sbp_msg_ssr_flag_high_level_dep_a_encode_internal(
  ...
  for (size_t i = 0; i < SBP_MSG_SSR_FLAG_HIGH_LEVEL_DEP_A_RESERVED_MAX; i++) {
...
s8 sbp_msg_ssr_flag_high_level_dep_a_encode(
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_flag_high_level_dep_a_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
bool sbp_msg_ssr_flag_high_level_dep_a_decode_internal(
  ...
  for (uint8_t i = 0; i < SBP_MSG_SSR_FLAG_HIGH_LEVEL_DEP_A_RESERVED_MAX; i++) {
...
s8 sbp_msg_ssr_flag_high_level_dep_a_decode(
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_flag_high_level_dep_a_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_flag_high_level_dep_a_send(
    sbp_state_t *s, u16 sender_id,
    const sbp_msg_ssr_flag_high_level_dep_a_t *msg, sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_flag_high_level_dep_a_encode(payload, sizeof(payload),
                                                    &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(s, SbpMsgSsrFlagHighLevelDepA, sender_id,
                                      payload_len, payload, write);
}

int sbp_msg_ssr_flag_high_level_dep_a_cmp(
...
int sbp_msg_ssr_flag_high_level_cmp(const sbp_msg_ssr_flag_high_level_t *a,
  ...
  ret = sbp_gps_time_sec_cmp(&a->sat_corr_time, &b->sat_corr_time);
  if (ret != 0) {
    return ret;
  }
