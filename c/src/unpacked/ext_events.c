...
s8 sbp_send_sbp_msg_ext_event_t(struct sbp_state *s, u16 sender_id, const sbp_msg_ext_event_t *msg, s32 (*write)(u8 *buff, u32 n, void *context))
{
  ...
  uint8_t payload_len;
  s8 ret = sbp_pack_sbp_msg_ext_event_t(payload, sizeof(payload), &payload_len, msg);
  if (ret != SBP_OK) { return ret; }
  return sbp_send_message(s, SBP_MSG_EXT_EVENT, sender_id, payload_len, payload, write);
}
