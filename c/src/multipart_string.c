...
bool sbp_multipart_string_valid(const sbp_multipart_string_t *s,
                                ...
                                uint8_t max_sections)
{
  if (s->packed_len > max_packed_len) return false;
  if (s->n_sections < min_sections) return false;
  if (s->n_sections > max_sections) return false;
  return true;
}
...
uint8_t sbp_multipart_string_packed_len(const sbp_multipart_string_t *s,
                                        ...
                                        uint8_t max_sections)
{
  if (!sbp_multipart_string_valid(s, max_packed_len, min_sections, max_sections)) return 0;
  return s->packed_len;
}
...
bool sbp_multipart_string_append(sbp_multipart_string_t *s,
                                 ...
                                 uint8_t max_sections)
{
  if (!sbp_multipart_string_valid(s, max_packed_len, min_sections, max_sections)) return false;
  if (s->n_sections == max_sections) return false;
...
const char *sbp_multipart_string_get_section(const sbp_multipart_string_t *s,
                                             ...
                                             uint8_t max_sections)
{
  if (!sbp_multipart_string_valid(s, max_packed_len, min_sections, max_sections)) return NULL;
