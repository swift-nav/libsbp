...
s8 sbp_msg_ssr_orbit_clock_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_ORBIT_CLOCK, sender_id,
...
s8 sbp_msg_ssr_code_biases_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_CODE_BIASES, sender_id,
                                      payload_len, payload, write);
}
...
s8 sbp_msg_ssr_phase_biases_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_PHASE_BIASES, sender_id,
                                      payload_len, payload, write);
}
...
bool sbp_msg_ssr_stec_correction_dep_encode_internal(
    sbp_encode_ctx_t *ctx, const sbp_msg_ssr_stec_correction_dep_t *msg) {
...
s8 sbp_msg_ssr_stec_correction_dep_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_stec_correction_dep_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_stec_correction_dep_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_stec_correction_dep_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_stec_correction_dep_send(
    sbp_state_t *s, u16 sender_id, const sbp_msg_ssr_stec_correction_dep_t *msg,
    sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_stec_correction_dep_encode(payload, sizeof(payload),
                                                  &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_STEC_CORRECTION_DEP,
                                      sender_id, payload_len, payload, write);
}

int sbp_msg_ssr_stec_correction_dep_cmp(
    const sbp_msg_ssr_stec_correction_dep_t *a,
    const sbp_msg_ssr_stec_correction_dep_t *b) {
  int ret = 0;
...
s8 sbp_bounds_header_encode(uint8_t *buf, uint8_t len, uint8_t *n_written,
                            const sbp_bounds_header_t *msg) {
  sbp_encode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_bounds_header_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
s8 sbp_bounds_header_decode(const uint8_t *buf, uint8_t len, uint8_t *n_read,
                            sbp_bounds_header_t *msg) {
  sbp_decode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_bounds_header_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
int sbp_bounds_header_cmp(const sbp_bounds_header_t *a,
                          const sbp_bounds_header_t *b) {
  int ret = 0;
...
bool sbp_msg_ssr_stec_correction_encode_internal(
  ...
  if (!sbp_u8_encode(ctx, &msg->ssr_iod_atmo)) {
    return false;
  }
  if (!sbp_u16_encode(ctx, &msg->tile_set_id)) {
    return false;
...
bool sbp_msg_ssr_stec_correction_decode_internal(
  ...
  if (!sbp_u8_decode(ctx, &msg->ssr_iod_atmo)) {
    return false;
  }
  if (!sbp_u16_decode(ctx, &msg->tile_set_id)) {
    return false;
...
s8 sbp_msg_ssr_stec_correction_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_STEC_CORRECTION, sender_id,
                                      payload_len, payload, write);
}
...
int sbp_msg_ssr_stec_correction_cmp(const sbp_msg_ssr_stec_correction_t *a,
  ...
  ret = sbp_u8_cmp(&a->ssr_iod_atmo, &b->ssr_iod_atmo);
  if (ret != 0) {
    return ret;
  }
...
s8 sbp_msg_ssr_gridded_correction_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_GRIDDED_CORRECTION,
                                      sender_id, payload_len, payload, write);
}
...
bool sbp_stec_sat_element_integrity_encode_internal(
  ...
  if (!sbp_u8_encode(ctx, &msg->stec_bound_mu)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->stec_bound_sig)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->stec_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->stec_bound_sig_dot)) {
    return false;
  }
  return true;
}

s8 sbp_stec_sat_element_integrity_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_stec_sat_element_integrity_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
bool sbp_stec_sat_element_integrity_decode_internal(
  ...
  if (!sbp_stec_residual_decode_internal(ctx, &msg->stec_residual)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->stec_bound_mu)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->stec_bound_sig)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->stec_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->stec_bound_sig_dot)) {
    return false;
  }
  return true;
}

s8 sbp_stec_sat_element_integrity_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_stec_sat_element_integrity_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
int sbp_stec_sat_element_integrity_cmp(
    const sbp_stec_sat_element_integrity_t *a,
    const sbp_stec_sat_element_integrity_t *b) {
  int ret = 0;

  ret = sbp_stec_residual_cmp(&a->stec_residual, &b->stec_residual);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->stec_bound_mu, &b->stec_bound_mu);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->stec_bound_sig, &b->stec_bound_sig);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->stec_bound_mu_dot, &b->stec_bound_mu_dot);
  if (ret != 0) {
    return ret;
  }
...
bool sbp_msg_ssr_gridded_correction_bounds_encode_internal(
    sbp_encode_ctx_t *ctx, const sbp_msg_ssr_gridded_correction_bounds_t *msg) {
  if (!sbp_bounds_header_encode_internal(ctx, &msg->header)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->ssr_iod_atmo)) {
  ...
  if (!sbp_u8_encode(ctx, &msg->tropo_qi)) {
    return false;
  }
  ...
  for (size_t i = 0; i < msg->n_sats; i++) {
    if (!sbp_stec_sat_element_integrity_encode_internal(
            ctx, &msg->stec_sat_list[i])) {
      return false;
    }
  }
  return true;
...
s8 sbp_msg_ssr_gridded_correction_bounds_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_gridded_correction_bounds_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
bool sbp_msg_ssr_gridded_correction_bounds_decode_internal(
    sbp_decode_ctx_t *ctx, sbp_msg_ssr_gridded_correction_bounds_t *msg) {
  if (!sbp_bounds_header_decode_internal(ctx, &msg->header)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->ssr_iod_atmo)) {
  ...
  if (!sbp_u8_decode(ctx, &msg->tropo_qi)) {
    return false;
  }
  ...
  for (uint8_t i = 0; i < msg->n_sats; i++) {
    if (!sbp_stec_sat_element_integrity_decode_internal(
            ctx, &msg->stec_sat_list[i])) {
      return false;
    }
  }
...
s8 sbp_msg_ssr_gridded_correction_bounds_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_gridded_correction_bounds_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_gridded_correction_bounds_send(
    sbp_state_t *s, u16 sender_id,
    const sbp_msg_ssr_gridded_correction_bounds_t *msg, sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_gridded_correction_bounds_encode(
      payload, sizeof(payload), &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_GRIDDED_CORRECTION_BOUNDS,
                                      sender_id, payload_len, payload, write);
}

int sbp_msg_ssr_gridded_correction_bounds_cmp(
  ...
  ret = sbp_u8_cmp(&a->tropo_qi, &b->tropo_qi);
  if (ret != 0) {
    return ret;
  }
...
s8 sbp_msg_ssr_tile_definition_dep_a_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_tile_definition_dep_a_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_tile_definition_dep_a_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_tile_definition_dep_a_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_tile_definition_dep_a_send(
    sbp_state_t *s, u16 sender_id,
    const sbp_msg_ssr_tile_definition_dep_a_t *msg, sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_tile_definition_dep_a_encode(payload, sizeof(payload),
                                                    &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_TILE_DEFINITION_DEP_A,
                                      sender_id, payload_len, payload, write);
}

int sbp_msg_ssr_tile_definition_dep_a_cmp(
    const sbp_msg_ssr_tile_definition_dep_a_t *a,
    const sbp_msg_ssr_tile_definition_dep_a_t *b) {
  int ret = 0;
...
s8 sbp_msg_ssr_tile_definition_dep_b_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_tile_definition_dep_b_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_tile_definition_dep_b_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_tile_definition_dep_b_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_tile_definition_dep_b_send(
    sbp_state_t *s, u16 sender_id,
    const sbp_msg_ssr_tile_definition_dep_b_t *msg, sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_tile_definition_dep_b_encode(payload, sizeof(payload),
                                                    &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_TILE_DEFINITION_DEP_B,
                                      sender_id, payload_len, payload, write);
}

int sbp_msg_ssr_tile_definition_dep_b_cmp(
    const sbp_msg_ssr_tile_definition_dep_b_t *a,
    const sbp_msg_ssr_tile_definition_dep_b_t *b) {
  int ret = 0;
...
s8 sbp_msg_ssr_tile_definition_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_TILE_DEFINITION, sender_id,
                                      payload_len, payload, write);
}
...
s8 sbp_msg_ssr_satellite_apc_dep_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_SATELLITE_APC_DEP,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_satellite_apc_send(sbp_state_t *s, u16 sender_id,
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_SATELLITE_APC, sender_id,
                                      payload_len, payload, write);
}
...
s8 sbp_msg_ssr_orbit_clock_dep_a_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_ORBIT_CLOCK_DEP_A,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_stec_correction_dep_a_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_STEC_CORRECTION_DEP_A,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_gridded_correction_no_std_dep_a_send(
  ...
  return sbp_internal_forward_payload(
      s, SBP_MSG_SSR_GRIDDED_CORRECTION_NO_STD_DEP_A, sender_id, payload_len,
      payload, write);
}
...
s8 sbp_msg_ssr_gridded_correction_dep_a_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_GRIDDED_CORRECTION_DEP_A,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_grid_definition_dep_a_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_GRID_DEFINITION_DEP_A,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_orbit_clock_bounds_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_ORBIT_CLOCK_BOUNDS,
                                      sender_id, payload_len, payload, write);
}
...
s8 sbp_msg_ssr_code_phase_biases_bounds_send(
  ...
  return sbp_internal_forward_payload(s, SBP_MSG_SSR_CODE_PHASE_BIASES_BOUNDS,
                                      sender_id, payload_len, payload, write);
}
...
bool sbp_orbit_clock_bound_degradation_encode_internal(
  ...
  if (!sbp_u8_encode(ctx, &msg->orb_radial_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->orb_along_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->orb_cross_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->orb_radial_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->orb_along_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->orb_cross_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->clock_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_encode(ctx, &msg->clock_bound_sig_dot)) {
    return false;
  }
  return true;
}

s8 sbp_orbit_clock_bound_degradation_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_orbit_clock_bound_degradation_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
bool sbp_orbit_clock_bound_degradation_decode_internal(
  ...
  if (!sbp_u8_decode(ctx, &msg->orb_radial_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->orb_along_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->orb_cross_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->orb_radial_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->orb_along_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->orb_cross_bound_sig_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->clock_bound_mu_dot)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->clock_bound_sig_dot)) {
    return false;
  }
  return true;
}

s8 sbp_orbit_clock_bound_degradation_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_orbit_clock_bound_degradation_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
int sbp_orbit_clock_bound_degradation_cmp(
    const sbp_orbit_clock_bound_degradation_t *a,
    const sbp_orbit_clock_bound_degradation_t *b) {
  int ret = 0;

  ret = sbp_u8_cmp(&a->orb_radial_bound_mu_dot, &b->orb_radial_bound_mu_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->orb_along_bound_mu_dot, &b->orb_along_bound_mu_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->orb_cross_bound_mu_dot, &b->orb_cross_bound_mu_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->orb_radial_bound_sig_dot, &b->orb_radial_bound_sig_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->orb_along_bound_sig_dot, &b->orb_along_bound_sig_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->orb_cross_bound_sig_dot, &b->orb_cross_bound_sig_dot);
  if (ret != 0) {
    return ret;
  }

  ret = sbp_u8_cmp(&a->clock_bound_mu_dot, &b->clock_bound_mu_dot);
  if (ret != 0) {
    return ret;
  }
...
bool sbp_msg_ssr_orbit_clock_bounds_degradation_encode_internal(
    sbp_encode_ctx_t *ctx,
  ...
  if (!sbp_u64_encode(ctx, &msg->sat_bitmask)) {
    return false;
  }
  if (!sbp_orbit_clock_bound_degradation_encode_internal(
          ctx, &msg->orbit_clock_bounds_degradation)) {
    return false;
  }
  return true;
}

s8 sbp_msg_ssr_orbit_clock_bounds_degradation_encode(
    uint8_t *buf, uint8_t len, uint8_t *n_written,
    const sbp_msg_ssr_orbit_clock_bounds_degradation_t *msg) {
  sbp_encode_ctx_t ctx;
  ctx.buf = buf;
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_orbit_clock_bounds_degradation_encode_internal(&ctx, msg)) {
    return SBP_ENCODE_ERROR;
  }
  if (n_written != NULL) {
    *n_written = (uint8_t)ctx.offset;
...
bool sbp_msg_ssr_orbit_clock_bounds_degradation_decode_internal(
    sbp_decode_ctx_t *ctx, sbp_msg_ssr_orbit_clock_bounds_degradation_t *msg) {
  if (!sbp_bounds_header_decode_internal(ctx, &msg->header)) {
    return false;
  }
  if (!sbp_u8_decode(ctx, &msg->ssr_iod)) {
  ...
  if (!sbp_u64_decode(ctx, &msg->sat_bitmask)) {
    return false;
  }
  if (!sbp_orbit_clock_bound_degradation_decode_internal(
          ctx, &msg->orbit_clock_bounds_degradation)) {
    return false;
  }
  return true;
...
s8 sbp_msg_ssr_orbit_clock_bounds_degradation_decode(
    const uint8_t *buf, uint8_t len, uint8_t *n_read,
  ...
  ctx.buf_len = len;
  ctx.offset = 0;
  if (!sbp_msg_ssr_orbit_clock_bounds_degradation_decode_internal(&ctx, msg)) {
    return SBP_DECODE_ERROR;
  }
  if (n_read != NULL) {
    *n_read = (uint8_t)ctx.offset;
...
s8 sbp_msg_ssr_orbit_clock_bounds_degradation_send(
    sbp_state_t *s, u16 sender_id,
    const sbp_msg_ssr_orbit_clock_bounds_degradation_t *msg,
    sbp_write_fn_t write) {
  uint8_t payload[SBP_MAX_PAYLOAD_LEN];
  uint8_t payload_len;
  s8 ret = sbp_msg_ssr_orbit_clock_bounds_degradation_encode(
      payload, sizeof(payload), &payload_len, msg);
  if (ret != SBP_OK) {
    return ret;
  }
  return sbp_internal_forward_payload(
      s, SBP_MSG_SSR_ORBIT_CLOCK_BOUNDS_DEGRADATION, sender_id, payload_len,
      payload, write);
}

int sbp_msg_ssr_orbit_clock_bounds_degradation_cmp(
    const sbp_msg_ssr_orbit_clock_bounds_degradation_t *a,
    const sbp_msg_ssr_orbit_clock_bounds_degradation_t *b) {
  int ret = 0;
  ...
  ret = sbp_u64_cmp(&a->sat_bitmask, &b->sat_bitmask);
  if (ret != 0) {
    return ret;
  }
