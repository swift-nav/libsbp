...
{
  if (!sbp_unterminated_string_valid(s, max_packed_len)) return 0;
  return s->len;
}
...
bool sbp_unterminated_string_printf(sbp_unterminated_string_t *s, uint8_t max_packed_len, const char *fmt, ...)
{
  ...
  if ((size_t)len >= max_packed_len + 1u) { return false; }
...
{
  if (!sbp_unterminated_string_valid(s, max_packed_len)) { return NULL; }
  return s->data;
}
...
uint8_t sbp_unterminated_string_unpack(sbp_unterminated_string_t *s, uint8_t max_packed_len, const uint8_t *buf, uint8_t buf_len)
{
  ...
  if (copy_len > max_packed_len) copy_len = max_packed_len;
