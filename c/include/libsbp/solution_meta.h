...
static inline size_t sbp_packed_size_sbp_msg_soln_meta_dep_a_t(
    const sbp_msg_soln_meta_dep_a_t *msg) {
  (void)msg;
  return 0 + sizeof(msg->pdop) + sizeof(msg->hdop) + sizeof(msg->vdop) +
         sizeof(msg->n_sats) + sizeof(msg->age_corrections) +
         sizeof(msg->alignment_status) + sizeof(msg->last_used_gnss_pos_tow) +
         sizeof(msg->last_used_gnss_vel_tow) +
         (msg->n_sol_in * (0 + sizeof(msg->sol_in[0].sensor_type) +
                           sizeof(msg->sol_in[0].flags)));
}

static inline bool sbp_pack_sbp_msg_soln_meta_dep_a_t(
    u8 *buf, size_t len, const sbp_msg_soln_meta_dep_a_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
  ...
  offset += 4;
  for (size_t msgsol_in_idx = 0; msgsol_in_idx < (size_t)msg->n_sol_in;
       msgsol_in_idx++) {
    if (offset + 1 > len) {
      return false;
    }
    u8 msgsol_inmsgsol_in_idxsensor_type =
        msg->sol_in[msgsol_in_idx].sensor_type;
...
static inline bool sbp_unpack_sbp_msg_soln_meta_dep_a_t(
    const u8 *buf, size_t len, sbp_msg_soln_meta_dep_a_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
  ...
  for (size_t msgsol_in_idx = 0; msgsol_in_idx < msg->n_sol_in;
       msgsol_in_idx++) {
    if (offset + 1 > len) {
      return false;
    }
...
static inline size_t sbp_packed_size_sbp_msg_soln_meta_t(
    const sbp_msg_soln_meta_t *msg) {
  (void)msg;
  return 0 + sizeof(msg->tow) + sizeof(msg->pdop) + sizeof(msg->hdop) +
         sizeof(msg->vdop) + sizeof(msg->age_corrections) +
         sizeof(msg->age_gnss) +
         (msg->n_sol_in * (0 + sizeof(msg->sol_in[0].sensor_type) +
                           sizeof(msg->sol_in[0].flags)));
}

static inline bool sbp_pack_sbp_msg_soln_meta_t(
    u8 *buf, size_t len, const sbp_msg_soln_meta_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
...
static inline bool sbp_unpack_sbp_msg_soln_meta_t(const u8 *buf, size_t len,
