...
#define SBP_ODOMETRY_FLAGS_VEHICLE_METADATA_SET(flags, val)           \
  do {                                                                \
    ((flags) |= (((val) & (SBP_ODOMETRY_FLAGS_VEHICLE_METADATA_MASK)) \
                 << (SBP_ODOMETRY_FLAGS_VEHICLE_METADATA_SHIFT)));    \
  } while (0)
...
#define SBP_ODOMETRY_FLAGS_VELOCITY_SOURCE_SET(flags, val)           \
  do {                                                               \
    ((flags) |= (((val) & (SBP_ODOMETRY_FLAGS_VELOCITY_SOURCE_MASK)) \
                 << (SBP_ODOMETRY_FLAGS_VELOCITY_SOURCE_SHIFT)));    \
  } while (0)
...
#define SBP_ODOMETRY_FLAGS_TIME_SOURCE_SET(flags, val)           \
  do {                                                           \
    ((flags) |= (((val) & (SBP_ODOMETRY_FLAGS_TIME_SOURCE_MASK)) \
                 << (SBP_ODOMETRY_FLAGS_TIME_SOURCE_SHIFT)));    \
  } while (0)
...
static inline size_t sbp_packed_size_sbp_msg_odometry_t(
    const sbp_msg_odometry_t *msg) {
  (void)msg;
  return 0 + sizeof(msg->tow) + sizeof(msg->velocity) + sizeof(msg->flags);
}

static inline bool sbp_pack_sbp_msg_odometry_t(u8 *buf, size_t len,
                                               const sbp_msg_odometry_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
...
static inline bool sbp_unpack_sbp_msg_odometry_t(const u8 *buf, size_t len,
...
#define SBP_WHEELTICK_FLAGS_VEHICLE_METADATA_SET(flags, val)           \
  do {                                                                 \
    ((flags) |= (((val) & (SBP_WHEELTICK_FLAGS_VEHICLE_METADATA_MASK)) \
                 << (SBP_WHEELTICK_FLAGS_VEHICLE_METADATA_SHIFT)));    \
  } while (0)
...
#define SBP_WHEELTICK_FLAGS_SYNCHRONIZATION_TYPE_SET(flags, val)           \
  do {                                                                     \
    ((flags) |= (((val) & (SBP_WHEELTICK_FLAGS_SYNCHRONIZATION_TYPE_MASK)) \
                 << (SBP_WHEELTICK_FLAGS_SYNCHRONIZATION_TYPE_SHIFT)));    \
  } while (0)
...
#define SBP_WHEELTICK_FLAGS_SYNCHRONIZATION_TYPE_LOCAL_CPU_TIME_IN_NOMINAL_MICROSECONDS \
  (2)
typedef struct {
  /**
...
static inline size_t sbp_packed_size_sbp_msg_wheeltick_t(
    const sbp_msg_wheeltick_t *msg) {
  (void)msg;
  return 0 + sizeof(msg->time) + sizeof(msg->flags) + sizeof(msg->source) +
         sizeof(msg->ticks);
}

static inline bool sbp_pack_sbp_msg_wheeltick_t(
    u8 *buf, size_t len, const sbp_msg_wheeltick_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
...
static inline bool sbp_unpack_sbp_msg_wheeltick_t(const u8 *buf, size_t len,
