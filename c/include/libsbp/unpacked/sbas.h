...
#define SBP_SBAS_RAW_SID_CODE__GET(flags) (((flags) >> SBP_SBAS_RAW_SID_CODE__SHIFT) & SBP_SBAS_RAW_SID_CODE__MASK)
#define SBP_SBAS_RAW_SID_CODE__SET(flags, val) \
  do \
  { \
    ((flags) |= (((val) & (SBP_SBAS_RAW_SID_CODE__MASK)) << (SBP_SBAS_RAW_SID_CODE__SHIFT))); \
  } while (0)
