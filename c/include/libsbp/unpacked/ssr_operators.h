...
static inline bool operator==(const sbp_phase_biases_content_t &a, const sbp_phase_biases_content_t &b)
{
  ...
  if (a.integer_indicator != b.integer_indicator)
  {
    return false;
  }

  if (a.widelane_integer_indicator != b.widelane_integer_indicator)
  {
    return false;
  }

  if (a.discontinuity_counter != b.discontinuity_counter)
  {
    return false;
  }
...
static inline bool operator==(const sbp_gridded_correction_header_t &a, const sbp_gridded_correction_header_t &b)
{
  ...
  if (a.tropo_quality_indicator != b.tropo_quality_indicator)
  {
    return false;
  }
...
static inline bool operator==(const sbp_stec_sat_element_t &a, const sbp_stec_sat_element_t &b)
{
  ...
  if (a.sv_id.satId != b.sv_id.satId)
  {
    return false;
  }

  if (a.sv_id.constellation != b.sv_id.constellation)
  {
    return false;
  }

  if (a.stec_quality_indicator != b.stec_quality_indicator)
  {
    return false;
  }
  for (size_t stec_coeff_idx = 0; stec_coeff_idx < 4; stec_coeff_idx++)
  {
    ...
    if (a.stec_coeff[stec_coeff_idx] != b.stec_coeff[stec_coeff_idx])
    {
      return false;
    }
  }
...
#endif
#ifdef __cplusplus
static inline bool operator==(const sbp_tropospheric_delay_correction_no_std_t &a,
...
static inline bool operator==(const sbp_grid_element_no_std_t &a, const sbp_grid_element_no_std_t &b)
{
  ...
  if (a.tropo_delay_correction.hydro != b.tropo_delay_correction.hydro)
  {
    return false;
  }

  if (a.tropo_delay_correction.wet != b.tropo_delay_correction.wet)
  {
    return false;
  }
  ...
  for (size_t stec_residuals_idx = 0; stec_residuals_idx < (size_t)a.stec_residuals_count; stec_residuals_idx++)
  {
    ...
    if (a.stec_residuals[stec_residuals_idx].sv_id.constellation !=
        b.stec_residuals[stec_residuals_idx].sv_id.constellation)
    {
      return false;
    }

    if (a.stec_residuals[stec_residuals_idx].residual != b.stec_residuals[stec_residuals_idx].residual)
    {
      return false;
    }
  }
...
static inline bool operator==(const sbp_grid_element_t &a, const sbp_grid_element_t &b)
{
  ...
  if (a.tropo_delay_correction.stddev != b.tropo_delay_correction.stddev)
  {
    return false;
  }
  if (a.stec_residuals_count != b.stec_residuals_count)
  ...
  for (size_t stec_residuals_idx = 0; stec_residuals_idx < (size_t)a.stec_residuals_count; stec_residuals_idx++)
  {
    ...
    if (a.stec_residuals[stec_residuals_idx].stddev != b.stec_residuals[stec_residuals_idx].stddev)
    {
      return false;
    }
  }
...
static inline bool operator==(const sbp_satellite_apc_t &a, const sbp_satellite_apc_t &b)
{
  ...
  for (size_t pco_idx = 0; pco_idx < 3; pco_idx++)
  {
    ...
    if (a.pco[pco_idx] != b.pco[pco_idx])
    {
      return false;
    }
  }
  for (size_t pcv_idx = 0; pcv_idx < 21; pcv_idx++)
  {
    ...
    if (a.pcv[pcv_idx] != b.pcv[pcv_idx])
    {
      return false;
    }
  }
...
static inline bool operator==(const sbp_grid_definition_header_dep_a_t &a, const sbp_grid_definition_header_dep_a_t &b)
{
  ...
  if (a.region_size_inverse != b.region_size_inverse)
  {
    return false;
  }

  if (a.area_width != b.area_width)
  {
    return false;
  }

  if (a.lat_nw_corner_enc != b.lat_nw_corner_enc)
  {
    return false;
  }

  if (a.lon_nw_corner_enc != b.lon_nw_corner_enc)
  {
    return false;
  }
