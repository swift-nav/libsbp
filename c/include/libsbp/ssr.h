...
#define SBP_SSR_SATELLITE_APC_APC_SID_CODE__SET(flags, val)           \
  do {                                                                \
    ((flags) |= (((val) & (SBP_SSR_SATELLITE_APC_APC_SID_CODE__MASK)) \
                 << (SBP_SSR_SATELLITE_APC_APC_SID_CODE__SHIFT)));    \
  } while (0)
...
static inline size_t sbp_packed_size_sbp_msg_ssr_satellite_apc_t(
    const sbp_msg_ssr_satellite_apc_t *msg) {
  (void)msg;
  return 0 +
         (msg->n_apc *
          (0 +
           (0 + sizeof(msg->apc[0].sid.sat) + sizeof(msg->apc[0].sid.code)) +
           sizeof(msg->apc[0].sat_info) + sizeof(msg->apc[0].svn) +
           (3 * sizeof(msg->apc[0].pco[0])) +
           (21 * sizeof(msg->apc[0].pcv[0]))));
}

static inline bool sbp_pack_sbp_msg_ssr_satellite_apc_t(
    u8 *buf, size_t len, const sbp_msg_ssr_satellite_apc_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
  ...
  for (size_t msgapc_idx = 0; msgapc_idx < (size_t)msg->n_apc; msgapc_idx++) {
    ...
    offset += 2;
    for (size_t msgapcmsgapc_idxpco_idx = 0; msgapcmsgapc_idxpco_idx < 3;
         msgapcmsgapc_idxpco_idx++) {
      if (offset + 2 > len) {
        return false;
      }
      s16 msgapcmsgapc_idxpcomsgapcmsgapc_idxpco_idx =
          msg->apc[msgapc_idx].pco[msgapcmsgapc_idxpco_idx];
      msgapcmsgapc_idxpcomsgapcmsgapc_idxpco_idx =
          htole16(msgapcmsgapc_idxpcomsgapcmsgapc_idxpco_idx);
      memcpy(buf + offset, &msgapcmsgapc_idxpcomsgapcmsgapc_idxpco_idx, 2);
      offset += 2;
    }
    for (size_t msgapcmsgapc_idxpcv_idx = 0; msgapcmsgapc_idxpcv_idx < 21;
         msgapcmsgapc_idxpcv_idx++) {
      if (offset + 1 > len) {
        return false;
      }
      s8 msgapcmsgapc_idxpcvmsgapcmsgapc_idxpcv_idx =
          msg->apc[msgapc_idx].pcv[msgapcmsgapc_idxpcv_idx];
...
static inline bool sbp_unpack_sbp_msg_ssr_satellite_apc_t(
    const u8 *buf, size_t len, sbp_msg_ssr_satellite_apc_t *msg) {
  size_t offset = 0;
  (void)offset;
  (void)buf;
  (void)len;
  ...
  for (size_t msgapc_idx = 0; msgapc_idx < msg->n_apc; msgapc_idx++) {
    ...
    for (size_t msgapcmsgapc_idxpco_idx = 0; msgapcmsgapc_idxpco_idx < 3;
      ...
      memcpy(&msg->apc[msgapc_idx].pco[msgapcmsgapc_idxpco_idx], buf + offset,
             2);
      msg->apc[msgapc_idx].pco[msgapcmsgapc_idxpco_idx] =
          le16toh(msg->apc[msgapc_idx].pco[msgapcmsgapc_idxpco_idx]);
      offset += 2;
    }
    for (size_t msgapcmsgapc_idxpcv_idx = 0; msgapcmsgapc_idxpcv_idx < 21;
         msgapcmsgapc_idxpcv_idx++) {
      ...
      memcpy(&msg->apc[msgapc_idx].pcv[msgapcmsgapc_idxpcv_idx], buf + offset,
             1);
      offset += 1;
    }
  }
