...
{
  (void)msg;
  return 0 + sizeof(msg->cn0) + sizeof(msg->cp) + sizeof(msg->cf) + (0 + sizeof(msg->sid.sat) + sizeof(msg->sid.code));
}
...
static inline bool operator==(const sbp_msg_acq_result_t &a, const sbp_msg_acq_result_t &b)
{
  ...
  if (fabs(a.cn0 - b.cn0) > 0.001)
  {
    return false;
  }

  if (fabs(a.cp - b.cp) > 0.001)
  {
    return false;
  }

  if (fabs(a.cf - b.cf) > 0.001)
  {
    return false;
  }
...
{
  (void)msg;
  return 0 + sizeof(msg->snr) + sizeof(msg->cp) + sizeof(msg->cf) + sizeof(msg->prn);
}
...
static inline size_t sbp_packed_size_sbp_msg_acq_sv_profile_t(const sbp_msg_acq_sv_profile_t *msg)
{
  ...
  return 0 + (msg->n_acq_sv_profile *
              (0 + sizeof(msg->acq_sv_profile[0].job_type) + sizeof(msg->acq_sv_profile[0].status) +
               ...
               sizeof(msg->acq_sv_profile[0].bin_width) + sizeof(msg->acq_sv_profile[0].timestamp) +
               sizeof(msg->acq_sv_profile[0].time_spent) + sizeof(msg->acq_sv_profile[0].cf_min) +
               sizeof(msg->acq_sv_profile[0].cf_max) + sizeof(msg->acq_sv_profile[0].cf) +
...
static inline bool sbp_pack_sbp_msg_acq_sv_profile_t(u8 *buf, size_t len, const sbp_msg_acq_sv_profile_t *msg)
{
  ...
  for (size_t msgacq_sv_profile_idx = 0; msgacq_sv_profile_idx < (size_t)msg->n_acq_sv_profile; msgacq_sv_profile_idx++)
  {
    ...
    u32 msgacq_sv_profilemsgacq_sv_profile_idxtime_spent =
        htole32(msg->acq_sv_profile[msgacq_sv_profile_idx].time_spent);
    ...
    u32 msgacq_sv_profilemsgacq_sv_profile_idxcf_min =
        htole32(*(const u32 *)&msg->acq_sv_profile[msgacq_sv_profile_idx].cf_min);
    ...
    u32 msgacq_sv_profilemsgacq_sv_profile_idxcf_max =
        htole32(*(const u32 *)&msg->acq_sv_profile[msgacq_sv_profile_idx].cf_max);
    ...
    u32 msgacq_sv_profilemsgacq_sv_profile_idxcf =
        htole32(*(const u32 *)&msg->acq_sv_profile[msgacq_sv_profile_idx].cf);
...
static inline bool operator==(const sbp_msg_acq_sv_profile_t &a, const sbp_msg_acq_sv_profile_t &b)
{
  ...
  for (size_t acq_sv_profile_idx = 0; acq_sv_profile_idx < (size_t)a.n_acq_sv_profile; acq_sv_profile_idx++)
  {
    ...
    if (a.acq_sv_profile[acq_sv_profile_idx].status != b.acq_sv_profile[acq_sv_profile_idx].status)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].cn0 != b.acq_sv_profile[acq_sv_profile_idx].cn0)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].int_time != b.acq_sv_profile[acq_sv_profile_idx].int_time)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].sid.sat != b.acq_sv_profile[acq_sv_profile_idx].sid.sat)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].sid.code != b.acq_sv_profile[acq_sv_profile_idx].sid.code)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].bin_width != b.acq_sv_profile[acq_sv_profile_idx].bin_width)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].timestamp != b.acq_sv_profile[acq_sv_profile_idx].timestamp)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].time_spent != b.acq_sv_profile[acq_sv_profile_idx].time_spent)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].cf_min != b.acq_sv_profile[acq_sv_profile_idx].cf_min)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].cf_max != b.acq_sv_profile[acq_sv_profile_idx].cf_max)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].cf != b.acq_sv_profile[acq_sv_profile_idx].cf)
    {
      return false;
    }

    if (a.acq_sv_profile[acq_sv_profile_idx].cp != b.acq_sv_profile[acq_sv_profile_idx].cp)
    {
      return false;
    }
  }
...
static inline bool operator==(const sbp_msg_acq_sv_profile_dep_t &a, const sbp_msg_acq_sv_profile_dep_t &b)
{
  ...
  for (size_t acq_sv_profile_idx = 0; acq_sv_profile_idx < (size_t)a.n_acq_sv_profile; acq_sv_profile_idx++)
  {
    ...
    if (a.acq_sv_profile[acq_sv_profile_idx].sid.reserved != b.acq_sv_profile[acq_sv_profile_idx].sid.reserved)
    {
      return false;
    }
