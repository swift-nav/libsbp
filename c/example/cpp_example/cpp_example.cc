...
class ECEFHandler
    : private sbp::MessageHandler<msg_gps_time_t, msg_pos_ecef_t> {
public:
  ECEFHandler(sbp::State *state)
  ...
  void handle_sbp_msg(uint16_t sender_id, uint8_t message_length,
    ...
    (void)sender_id;
    (void)message_length;
    std::cout << "Received new GPS_TME message with WN = " << msg.wn
              << ", TOW = " << msg.tow << "\n";
  }

  void handle_sbp_msg(uint16_t sender_id, uint8_t message_length,
                      const msg_pos_ecef_t &msg) {
    (void)sender_id;
    (void)message_length;
...
class LLHFrameHandler
    : private sbp::FrameHandler<msg_gps_time_t, msg_pos_llh_t> {
public:
  LLHFrameHandler(sbp::State *state)
      : sbp::FrameHandler<msg_gps_time_t, msg_pos_llh_t>(state) {}
  ...
  void handle_sbp_frame(uint16_t sender_id, uint16_t msg_type,
    ...
    if (msg_type == SBP_MSG_POS_LLH) {
      std::cout << "Received new POS_LLH frame, payload length "
                << (int)payload_len << ", frame length " << frame_len << "\n";
    } else if (msg_type == SBP_MSG_GPS_TIME) {
      std::cout << "Received new GPS_TIME frame, payload length "
                << (int)payload_len << ", frame length " << frame_len << "\n";
    }
  }
};
...
public:
  ...
  void handle_sbp_frame(uint16_t sender_id, uint16_t msg_type,
    ...
    (void)payload;
    (void)frame;
    std::cout << "Received new frame, message type " << msg_type
              << ", payload length " << (int)payload_len << ", frame length "
              << frame_len << "\n";
  }
};
