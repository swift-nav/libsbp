...
def test_corrupted():
    ...
    with os.fdopen(fd_corrupted, 'wb') as fp_corrupted:
        with open("test_data/benchmark.sbp", 'rb') as fp_orig:
            while True:
                # read random amount
                buf = fp_orig.read(random.randrange(8192))
                if buf is None or len(buf) == 0:
                    break
                fp_corrupted.write(buf)
                if random.random() < 0.5: # drop bytes
                    fp_orig.read(random.randrange(3))
                else: # write garbage
                    fp_corrupted.write(bytes(chr(random.randrange(127)), encoding='ascii'))
