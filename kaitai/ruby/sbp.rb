...
class Sbp < Kaitai::Struct::Struct
  ...
  class SbpMessage < Kaitai::Struct::Struct
    ...
    def _read
      ...
      raise Kaitai::Struct::ValidationNotEqualError.new([85].pack('C*'), preamble, _io, "/types/sbp_message/seq/0") if not preamble == [85].pack('C*')
      @msg_type = @_io.read_u2le
      @sender = @_io.read_u2le
      @length = @_io.read_u1
      case msg_type
      when 525
        @_raw_payload = @_io.read_bytes(length)
