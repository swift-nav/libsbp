...
data PackedObsContent = PackedObsContent
  ...
  , _packedObsContent_L  :: Int32
    -- ^ Phase Range minus pseudorange. Phaserange can be reconstructed by adding
    -- pseudorange to this value. This follows the convention from DF0012 or
    -- the DF042 field from the RTCM 10403.2 Ammendment 2 specification.
    -- Importantly, this assumes the phaserange has the  same sign as the
    -- pseudorange.
  , _packedObsContent_cn0 :: Word8
    -- ^ Carrier-to-Noise density
  , _packedObsContent_lock :: Word8
    -- ^ Lock indicator. This value gives an indication of the time for which a
    -- satellite has maintained continuous phase lock. Whenever a satellite
    -- signal has lost and regained lock, this  value is reset to zero. It is
    -- encoded according to DF402 from the RTCM 10403.2 Amendment 2
    -- specification.  Given a lock time (t) in milliseconds, the field value
    -- (n) is given by floor(log_2(t) - 4)  when t is greater than 32 ms or 0
    -- if (t) is less than 32 ms.  Conversely, given the field, n,  the lower
    -- range of possible  lock times is given by 2 ^ (n + 4) ms and the upper
    -- range is given by 2 ^ (n + 5) ms provided n is not 0.  If n is 0 the
    -- lower range is given to be 0 ms.
...
instance Binary PackedObsContent where
  get = do
    ...
    _packedObsContent_L <- liftM fromIntegral getWord32le
  ...
  put PackedObsContent {..} = do
    ...
    putWord32le $ fromIntegral _packedObsContent_L
...
-- phase observation here is phaserange - pseudorange. The observations should
-- be interoperable with 3rd party receivers and conform  with typical RTCMv3
-- GNSS observations.
data MsgObs = MsgObs
...
--
-- This ephemeris message has been deprecated to allow the ephemerides to be
-- masked separately from GNSS observations. The ephemeris message returns a
-- set of satellite orbits parameters that is used to calculate GPS satellite
-- position, velocity, and clock offset. Please see the Navstar GPS Space
-- Segment/Navigation user interfaces (ICD-GPS-200, Table 20-III) for more
-- details.
