...
-- GNSS metadata. Re. the age of the last received valid GNSS solution, the
-- highest two bits are Time status, which indicates whether age gnss can or
-- can not be used to retrieve time of measurement (noted TOM, aka time of
-- validity), i.e. to be used in conjuction with the tow field of other
-- navigation messages. If it can be used (value 1), substract from tow in
-- navigation message to retrieve the time of measurement. Useful before
-- alignment is complete in the Fusion Engine, when outputted solution is the
-- last received valid GNSS solution and its tow is not that of Time Of
-- Measurement.
