...
--
-- Metadata around the GNSS sensors involved in the fuzed solution. Note: Just
...
--
-- Metadata around the IMU sensors involved in the fuzed solution. Note: Just
-- to build descriptive tables in documentation and not actually used.
...
--
-- Metadata around the Odometry sensors involved in the fuzed solution. Note:
-- Just to build descriptive tables in documentation and not actually used.
...
data MsgSolnMeta = MsgSolnMeta
  ...
  , _msgSolnMeta_n_sats               :: !Word8
    ...
    -- Fuzed Solution.
  , _msgSolnMeta_age_of_corrections   :: !Word16
  ...
  , _msgSolnMeta_solution_inputs      :: ![SolutionInputType]
...
instance Binary MsgSolnMeta where
  get = do
    ...
    _msgSolnMeta_age_of_corrections <- getWord16le
    ...
    _msgSolnMeta_solution_inputs <- whileM (not <$> isEmpty) get
    pure MsgSolnMeta {..}
  ...
  put MsgSolnMeta {..} = do
    ...
    putWord16le _msgSolnMeta_age_of_corrections
    ...
    mapM_ put _msgSolnMeta_solution_inputs
