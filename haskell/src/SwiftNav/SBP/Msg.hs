...
data SBPMsg =
   ...
   | SBPMsgTrackingStateMe MsgTrackingStateMe Msg
...
instance Binary SBPMsg where
    ...
    if preamble /= msgSBPPreamble then get else
      decoder <$> get where
        decoder m@Msg {..}
          ...
          | _msgSBPType == msgTrackingStateMe = SBPMsgTrackingStateMe (decode (fromStrict (unBytes _msgSBPPayload))) m
  ...
  put sm = do
    ...
    encoder sm where
      ...
      encoder (SBPMsgTrackingStateMe _ m) = put m
  ...
  parseJSON obj@(Object o) = do
    ...
    decoder msgType payload where
      decoder msgType payload
        ...
        | msgType == msgTrackingStateMe = SBPMsgTrackingStateMe <$> pure (decode (fromStrict (unBytes payload))) <*> parseJSON obj
...
instance ToJSON SBPMsg where
  ...
  toJSON (SBPMsgTrackingStateMe n m) = toJSON n <<>> toJSON m
...
instance HasMsg SBPMsg where
  ...
  msg f (SBPMsgTrackingStateMe n m) = SBPMsgTrackingStateMe n <$> f m
