...
instance Binary SBPMsg where
    ...
    if preamble /= msgSBPPreamble then get else
      decoder <$> get where
        decoder m@Msg {..}
          ...
          | _msgSBPType == msgImuRawx = SBPMsgImuRawx (decode (fromStrict (unBytes _msgSBPPayload))) m
  ...
  put sm = do
    ...
    encoder sm where
      ...
      encoder (SBPMsgImuRawx _ m) = put m
  ...
  parseJSON obj@(Object o) = do
    ...
    decoder msgType payload where
      decoder msgType payload
        ...
        | msgType == msgImuRawx = SBPMsgImuRawx <$> pure (decode (fromStrict (unBytes payload))) <*> parseJSON obj
...
instance ToJSON SBPMsg where
  ...
  toJSON (SBPMsgImuRawx n m) = toJSON n <<>> toJSON m
...
instance HasMsg SBPMsg where
  ...
  msg f (SBPMsgImuRawx n m) = SBPMsgImuRawx n <$> f m
